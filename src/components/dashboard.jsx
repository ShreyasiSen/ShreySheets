...
function Dashboard() {
    ...
    const onDelete = async (e, spreadsheet) => {
        ...
        try {
            ...
            setSpreadsheets(prevSpreadsheets => prevSpreadsheets.filter(sheet => sheet._id !== spreadsheet._id));
            navigate('/dashboard');
            setShowPopup(true);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {spreadsheets.map(spreadsheet => (
                        <div key={spreadsheet._id} className="bg-white p-4 rounded-lg shadow-lg">
                            ...
                            <button className="bg-blue-500 text-white hover:bg-violet-400 px-2 py-1 rounded-lg"
                                onClick={() => navigate(`/spreadsheet/${spreadsheet._id}`)}>Open</button>
                            <button className="bg-red-500 ml-3 hover:bg-red-400 text-white px-2 py-1 rounded-lg" onClick={(e) => onDelete(e, spreadsheet)}>Delete</button>
                        </div>
                    ))}
                </div>
            )}
